...
namespace Battle
{
    public class PuppetManipulation : Skill
    {
        ...
        {
            StartCoroutine(SkillActionToEnemyCoroutine(target));
        }

        private IEnumerator SkillActionToEnemyCoroutine(GameObject target)
        {
            ...
            summonedPuppet.GetComponent<CharacterCore>().SetCharacterInfo(GetLevel());
            Debug.Log($"レベル{GetLevel()}の戦車召喚しました");
        ...
        {
            StartCoroutine(SkillActionforBuddyCoroutine(target));
        }

        private IEnumerator SkillActionforBuddyCoroutine(GameObject target)
        {
            GameObject summonedPuppet;
            if (GetLevel().Equals(4))
            {
